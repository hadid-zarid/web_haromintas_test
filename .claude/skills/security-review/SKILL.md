...
| Laravel Blade `{{ $variable }}` | Auto-escaped by default (`{!! !!}` is the unsafe form) |
| `User.objects.filter(id=input)` | ORM parameterizes queries |
| `User::where('id', $input)`, `->find($input)` | Eloquent parameterizes queries |
| `->whereRaw('col = ?', [$input])` | Raw SQL string is constant; input is bound |
| `cursor.execute("...%s", (input,))` | Parameterized query |
| `innerHTML = "<b>Loading...</b>"` | Constant string, no user input |

**Only flag these when:**
- Django: `{{ var|safe }}`, `{% autoescape off %}`, `mark_safe(user_input)`
- Laravel Blade: `{!! $user_input !!}`, `@php echo`, dangerous-attribute position
- React: `dangerouslySetInnerHTML={{__html: userInput}}`
- Vue: `v-html="userInput"`
- Django ORM: `.raw()`, `.extra()`, `RawSQL()` with string interpolation
- Eloquent: `DB::raw()`, `whereRaw`/`orderByRaw`/`selectRaw` with **interpolation** (not bindings), `orderBy($request->input(...))`
...
| `.php`, `artisan`, `composer.json`, `Illuminate\`, `laravel`, `inertia`, `.blade.php` | `languages/php.md` |
| `.go`, `go.mod` | `languages/go.md` |
| `.rs`, `Cargo.toml` | `languages/rust.md` |
| `.java`, `spring`, `@Controller` | `languages/java.md` |

> Note: only `python.md`, `javascript.md`, and `php.md` ship in this skill today. For other languages, fall back to the language-agnostic `references/` guides.
...
- `php.md` - Laravel + Inertia patterns (Eloquent/raw SQL, mass assignment, Blade, access control, Inertia prop leakage, CSRF exemptions, file/path handling)
- `go.md` - Go-specific security patterns *(not included)*
- `rust.md` - Rust unsafe blocks, FFI security *(not included)*
- `java.md` - Spring, Java EE patterns *(not included)*
